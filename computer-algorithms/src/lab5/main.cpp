...
Node* insertAVLBTree(Node* node, int data)
{
	...
	if (data < node->data)
		node->left  = insertAVLBTree(node->left, data);
	else
		node->right = insertAVLBTree(node->right, data);
...
Node* deleteAVLBTree(Node* root, int data)
{
	...
	// then it lies in left subtree
	if ( data < root->data )
		root->left = deleteAVLBTree(root->left, data);

	// If the key to be deleted is greater than the root's key,
	// then it lies in right subtree
	else if( data > root->data )
		root->right = deleteAVLBTree(root->right, data);
	...
	else
	{
		...
		else
		{
			...
			// Delete the inorder successor
			root->right = deleteAVLBTree(root->right, temp->data);
		}
	}
...
// Функция создает бинарное дерево поиска и возращает корневой узел дерева
Node* createAVLBTree(int *array, int size)
{
	Node* node = NULL;
	for (int i = 0; i < size; ++i) {
		node = insertAVLBTree(node, array[i]);
	}
	return node;
}
...
void exer4(int *data, int size) {
	cout << "Exer.4 - Create AVL B-Tree" << endl;
	Node* root = createAVLBTree(data, size);
	printPretty(root);
	free(root);
	cout << endl;
}

void exer5(int *data, int size) {
	cout << "Exer.5 - Delete nodes from AVL B-Tree" << endl;
	Node* root = createAVLBTree(data, size);

	cout << "Delete node with one children" << endl;
	deleteAVLBTree(root, 5);
	printPretty(root);
	cout << endl;

	cout << "Delete node with two childrens" << endl;
	deleteAVLBTree(root, 15);
	printPretty(root);

	cout << "Delete terminal node(root)" << endl;
	deleteAVLBTree(root, 35);
	printPretty(root);
...
int main()
{
	int data[] = {35, 55, 15, 50, 40, 2, 10, 45, 5, 30, 65};
